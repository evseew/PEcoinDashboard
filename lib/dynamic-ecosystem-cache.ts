...
class DynamicEcosystemCache {
  ...
  /**
   * Автоматическая инициализация экосистемы (ОТКЛЮЧЕНО для production)
   */
  async autoInitialize(): Promise<void> {
    // ОТКЛЮЧЕНО для production - дублирует запросы балансов с PublicDashboard
    if (process.env.NODE_ENV === 'production') {
      console.log('⚠️ DynamicEcosystemCache отключен для production (устранение дублирования запросов)')
      return
    }
    
    console.log(`🌐 Автоинициализация динамической экосистемы (development режим)`)
    ...
    // Первоначальная загрузка всех данных (только для development)
    await this.refreshAllData()
    
    // Запускаем периодические обновления (только для development)
