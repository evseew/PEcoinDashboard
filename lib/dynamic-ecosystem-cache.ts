...
class DynamicEcosystemCache {
  ...
  async refreshParticipants(): Promise<void> {
    ...
    try {
      ...
      // Получаем команды
      const teamsResponse = await fetch('/api/entities/teams')
      if (teamsResponse.ok) {
      ...
      // Получаем стартапы  
      const startupsResponse = await fetch('/api/entities/startups')
      ...
      const staffResponse = await fetch('/api/entities/staff')
      if (staffResponse.ok) {
